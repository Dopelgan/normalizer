...
# считается прозой даже при одном переключении гарнитуры.
_RUN_LIMIT = 6
...
def is_mangled_text(latex: str) -> bool:
    ...
    раза, а здесь — на каждой букве. Дополнительно засчитывается длинная
    цепочка одиночных букв: `\mathsf { K O O p a m H a r c a }` остаётся
    прозой, даже если команда в строке одна.
    """
    if not latex or "\\" not in latex:
        return False
    switches = font_switches(latex)
    if switches >= _FONT_LIMIT:
        return True
    return bool(switches) and _longest_letter_run(latex) >= _RUN_LIMIT
