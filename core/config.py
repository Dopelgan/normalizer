...
class Settings(BaseSettings):
    ...
    TZ: str = "UTC"
    LOG_LEVEL: str = "INFO"
    # Формат логов: json — строка на событие для сборщика логов, text — для
    # чтения глазами. Замеры времени этапов пишутся событиями `stage`, и
    # разбирать их удобнее в json.
    LOG_FORMAT: str = "json"
    # Логи доступа uvicorn на каждый опрос /health и /metrics забивают ленту.
    QUIET_ACCESS_LOG: bool = True
    # Порт метрик Prometheus у процессов без своего веб-сервера (воркеры).
    # У сервисов FastAPI метрики отдаёт ручка /metrics.
    ...
    # S3 у нас выдан только на чтение. Поэтому всё производное — вырезанные
    # картинки, выправленные растры, сырой JSON парсера — пишется не туда,
    # откуда пришёл документ, а в собственный том сервиса. Провайдер
    # хранилища при S3_READONLY=true читает s3:// из S3, а любую запись
    # уводит в ARTIFACTS_MOUNT. Ставить false стоит только тогда, когда
    # бакет действительно доступен на запись.
    S3_READONLY: bool = True
    ARTIFACTS_MOUNT: str = "/data/shared"
    # Временные файлы (выкачанный из S3 исходник для tesseract, промежуточные
    # растры poppler) держим в том же томе, а не на слое контейнера: он не
    # резиновый, и после падения воркера мусор виден и чистится.
    TEMP_DIR: str = "/data/shared/tmp"
    ...
    # Адрес Quality Gate. Цепочка приёма им больше не пользуется: оба этапа
    # идут в одной задаче, без запроса по сети. Переменная оставлена для
    # прямого, синхронного входа и внешних вызовов этого контейнера.
    ...
    # Очередь приёма. Приём вынесен из HTTP-запроса: ручка ставит задачу и
    # отвечает 202, вердикты забираются поллингом. Иначе пятьдесят сканов
    # без текстового слоя не укладываются ни в один разумный таймаут, а
    # потребитель, не дождавшись ответа, начинает слать запрос заново.
    CELERY_INTAKE_QUEUE: str = "intake"
    # Потолок времени на приём одного файла. Приём — это чтение, открытие
    # формата и быстрый OCR двух страниц: на сложном скане это минуты, но
    # не десятки минут.
    INTAKE_TASK_TIME_LIMIT: int = 900
    INTAKE_TASK_SOFT_TIME_LIMIT: int = 840
    # Сколько хранится состояние приёма для GET /intake/results/{id}.
    INTAKE_STATE_TTL_SECONDS: int = 86400
    # Передавать ли принятые файлы дальше, в нормализатор. Раньше вызов был
    # закомментирован в quality/api.py — теперь это настройка, и цепочка
    # включается без правки кода.
    INTAKE_FORWARD_TO_PARSER: bool = False
    ...
    QG_OCR_PAGES: int = 2
    # Доля распознанного текста ниже порога читаемости, после которой лист
    # считается прочитанным лишь частично. Среднее по листу вытягивается
    # крупной шапкой: документ, где читается заголовок и не читается тело,
    # проходил приём без единого предупреждения.
    QG_OCR_UNREADABLE_SHARE: float = 0.6

    # Полный OCR документа: потолок страниц и размер пачки рендера. Без
    # потолка `parse_all_pages` рендерил в память весь PDF целиком — на
    # техническом задании в несколько сотен листов это обрывало воркер по
    # памяти, а не возвращало результат. Усечение не молчит: оно попадает в
    # degraded разбора.
    OCR_FULL_MAX_PAGES: int = 50
    OCR_BATCH_PAGES: int = 5
    OCR_DPI: int = 200
    # Потолок длинной стороны страницы перед распознаванием. Сканы A3 с
    # разрешением 6600 px распознаются минутами, хотя тексту от такого
    # масштаба лучше не становится: tesseract работает по высоте строки.
    OCR_MAX_SIDE: int = 3500
    # Нижняя граница длинной стороны. Мелкий растр (вырезка из письма,
    # превью, скан 80x50) tesseract не читает вовсе: высота строки в нём
    # ниже порога распознавания, и на приёме такой файл уходил в карантин
    # с формулировкой «документ нечитаем», хотя человек его читает.
    OCR_MIN_SIDE: int = 900
    ...
    # DOC — двоичный Word 97-2003, своей библиотеки под него нет. Файл
    # конвертируется в DOCX разово, на лету, и дальше идёт штатным путём.
    DOC_CONVERT_ENABLED: bool = True
    SOFFICE_BIN: str = "soffice"
    DOC_CONVERT_TIMEOUT: int = 180
